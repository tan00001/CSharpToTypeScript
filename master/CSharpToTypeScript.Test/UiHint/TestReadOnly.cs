...
{
    public class PersonWithReadOnlyId
    {
        public static readonly string[] LocationOptions = new[] { "NY", "CA" };

        [UIHint("", "HTML", new object[] { "readOnly", true })]
    ...
    [TestClass]
    public class TestReadOnly : IDisposable
    {
        ...
        public void TestSelectOptions()
        {
            var ts = TypeScript.Definitions(new TsGeneratorWithForm(1, false))
               .For<PersonWithReadOnlyId>();

            var personTypeScript = ts.ToString();

            Assert.IsTrue(!string.IsNullOrEmpty(personTypeScript));

            var expectedData = Utilities.GetTestFormFileContents("UIHint", nameof(PersonWithReadOnlyId));
