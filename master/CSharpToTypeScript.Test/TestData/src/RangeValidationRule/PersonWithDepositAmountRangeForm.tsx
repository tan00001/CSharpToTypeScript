...
export const PersonResolver: Resolver<Person> = async (values) => {
	...
	if (values.depositAmount) {
		const propValue: number = parseFloat(values.depositAmount.replace(/\D/g, ''));
		if (isNaN(propValue) || propValue > 100000) {
			errors.depositAmount = {
				type: 'max',
				message: 'DepositAmount cannot exceed 100000.'
			};
		}
		if (isNaN(propValue) || propValue < 0) {
			errors.depositAmount = {
				type: 'min',
				message: 'DepositAmount cannot be less than 0.'
			};
		}
	}
	if (!values.firstName) {
...
export const PersonForm = (props: PersonFormData) => {
	...
	return <form onSubmit={handleSubmit(props.onSubmit)}>
		...
		<div className="row mb-3">
			<div className="form-group col-md-12">
				<label htmlFor={formId + "-depositAmount"}>DepositAmount:</label>
				<input type="text" className={getClassName(touchedFields.depositAmount, errors.depositAmount)} id={formId + "-depositAmount"} {...register("depositAmount")} />
				{getErrorMessage(errors.depositAmount)}
			</div>
		</div>
		<div className="row">
			<div className="form-group col-md-12">
