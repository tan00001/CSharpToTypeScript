...
export const PersonResolver: Resolver<Person> = async (values) => {
	...
	if (values.contractLength) {
		...
		const propValueForMax: number = parts.some(p => isNaN(p)) ? 172800001 : parts.reduce((total, part, index) =>
			(index < multipliers.length ? total + part * multipliers[index] : total), 0);
		if (propValueForMax > 172800000) {
			errors.contractLength = {
				type: 'max',
				message: 'Contract length must be between 100 and 200 days.'
			};
		}
		const propValueForMin: number = parts.some(p => isNaN(p)) ? 86399999 : parts.reduce((total, part, index) =>
			(index < multipliers.length ? total + part * multipliers[index] : total), 0);
		if (propValueForMin < 86400000) {
			errors.contractLength = {
				type: 'min',
				message: 'Contract length must be between 100 and 200 days.'
			};
